...
Meteor.methods({
	"createUserAccount": function(options) {
		...
		var userOptions = {};
		if(options.phone) userOptions.prphone = options.phone;
		...
		/*if(options.profile && options.profile.email) userOptions.email = options.profile.phone;*/
		if(options.role) role = options.role;

		console.log(options.profile);
		console.log(userOptions);
		// insert user
		var id = Accounts.createUser(userOptions);
...
Meteor.startup( function (){
	Roles.addUsersToRoles("HyjAebRLepXLva9DE", "admin");
});